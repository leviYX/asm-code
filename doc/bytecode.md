...
实际上java源码通过javac的前端编译器的编译之后，就会生成字节码。而这个字节码不仅仅是java的专属，实际上你只要按照这个字节码规范实现了你的字节码生成结果，就可以运行在jvm上，所以jvm虚拟机上可以跑很多语言，比如java，scala,kotlin等等。

而字节码的格式也很简单，**就是一个字节长的操作码**+多个操作参数，其中操作参数有时候可以没有。这个我们后面会见到很多这类。有一个或多个字节码构成的文件也就是我们要说的字节码文件。
...
之所以存在表这个结构是因为有些部分是不确定的，比如cp_info，这个结构表示常量池的内容，而一个类里面的常量是不固定的，所以他不能固定用某一个u2 u8这种表示，他是个数组。但是数组里面有多个，那怎么知道到哪个结束呢，毕竟也没有个开始符号之类的。所以每一个这种表的前一项就是他的长度，比如cp_info的前面是一个u2两字节长度的constant_pool_count，表示的是这个cp_info里面有多少项内容。类似设计网络协议的那种那种头体分离的机制。
...
如上图所示我们的constant_pool_count为0x 0016，对应的十进制数就是22。所以对应的我们的常量池表里面是从1-21，0号位置空余。也就是说我们的这个常量池里面有21项数据。
...
其实这个表N多年没变过了，因为他就那么多类型。其中15 16 18号标志的类型是从jdk7加进来的，java从jdk7之后开始大面积支持动态语言调用，具体可以去看jsr292，还有lanmda以及methodHandle等实现和功能。
...
- 全限定名：比如你有一个类，com/levi/Demo这个类的全部包名都列出来用/分隔，就是这个Demo类的全限定名。为了使多个连续的全限定名之间不会产生混淆，每一个类的全限定名后面会加一个;用来表示当前全限定名结束。

- 简单名称：简单名称指的是没有类型和参数修饰的方法或者字段名称，比如你的一个类中的add()方法和private int num这个字段的简单名称分别是add和num，就是去掉那些修饰符之后的东西。
...
- 补充说明：
  ...
  ~~~markdown
  补充说明:
  虚拟机在加载class字节码文件时才会进行动态链接，也就是一开始没加载的时候，那些符号引用都是静态的，也就是说，class字节码文件中不会保存各个方法和字段的最终内存布局信息，加载之后才会真正指向对应的空间或者对象，因此，这些字段和方法的符号引用不经过转换是无法直接被虚拟机使用的(因为此时引用并未生效，你也不知道引用的是谁，还没链接上呢)，当虚拟机运行的时候，需要从常量池中获取对应的符号引用，在类加载过程中的解析阶段将符号引用替换为直接引用，并且翻译到具体的内存地址中。
  ...
  这个说明看着其实还是云山雾罩的，因为我们还没有看到字节码下面的那些符号引用到底是啥。我这里可以先简单解释一下，这段话的意思其实是说，当我们的源码文件被javac编译之后成为class字节码文件，此时我们称之为他的布局是静态的，并没有加载到内存中，比如你的方法引用了一个字段，此时仅仅是这个方法在字节码中用字符串写了一个这个字段的符号，此时他还没加载到内存中，没真正引用到对象呢，不过这也是句废话，你都没动态加载到内存自然也不会引用对象，就是标记了一下路径啥的。
...
我们再来进一步细化一下字节码格式的表格。我们这个表格里面记录了每个类型的字节码的值，可以当成一个字典来查。

![87129b23-485d-425d-a97f-60e2321a8441](bytecode.assets/87129b23-485d-425d-a97f-60e2321a8441.png)



前面看到我们的constant_pool_count为0x 0016，对应的十进制数就是22。所以对应的我们的常量池表里面是从1-21，0号位置空余。也就是说我们的这个常量池里面有21项数据。也就是说我们从当前的常量池计数器00 16之后再往后找21项就是我们全部的常量池。但是常量池中可能有各种常量，每个内容的大小结构都不一样，那如何定位从哪里到哪里呢。下面我们就来一个一个的分析。

首先我们知道每个结构开始的一个字节是标记下面的结构的长度的，所以我们在0016后面看一个字节是0A,0A的十进制数是10，我们去表格中找到10对应的是CONSTANT_Methodref_info，而这个CONSTANT_Methodref_info有两个index分别是u2 u2，也就是说CONSTANT_Methodref_info类型的字节码占据了两个表项，长度分别是u2 u2一共四个字节。我们就从0A后面数四个字节。也就是00 04 00 12四个字节。所以常量池中的第一项我们就定位了。他是一个CONSTANT_Methodref_info类型的，并且后面跟了四个字节的数据。也就是0A 00 04 00 12 是我们常量池中的第一项。

下一个1字节是09，十进制数为9，我们去表格找到9代表的是CONSTANT_Fieldref_info。他有两个index分别是u2 u2，所以第二项也就出来了。

第三项也是一样的。

但是第四项我们看到它的长度是01，我们去表格找到01代表的是CONSTANT_utf8_info，也就是UTF-8编码的字符串。这个和前面不太一样，他不是按照index来占位的，他是

| length | u2   | UTF-8编码的字符串占用的字符数   |
| ------ | ---- | ------------------------------- |
| bytes  | u1   | 长度为length的UTF-8编码的字符串 |

他是因为字符串长度可能会很长没办法直接说占多少，他是先用一个u2的结构来存储字符串占的长度。而长度的单位就是bytes的u1，换言之我们这里这个如何解读呢。

01这个是tag长度没说的 。然后后面一个u2存储着长度，也就是00 03 也就是说这是一个3个字节的字符串，再往后数三个字节6E 75 6D 所以这就是这个1代表的结构。

然后我们来看第五项，首先tag是01，没说的了还是上面的规则，我们先找两个字节的字符串长度存储，00 01也就是说这个字符串占1个字节，那就再数一个字节49，所以这个第五项自然就是01 00 01 49。

然后我们再来看第六项tag还是01，再找两个字节的长度存储，00 06，也就是存的是6个字节，那就是往后数6个字节，

然后继续往下第七项依然是01，继续搞。

这样一直到01 00 09 44 65 6D 6F 2E 6A 61 76 61中间都是这个类型。

直到了0C 的十进制是12，对应的类型是CONSTANT_NameAndType_info，占据了两个u2的index，所以往后数两个2字节也就是四个字节，0C 00 07 00 08。

到了最后我们终于结束了21项内容。u1 + u2 + 16byte 01 00 10 6A 61 76 61 2F 6C 61 6E 67 2F 4F 62 6A 65 63 74

![image-20250211165818757](bytecode.assets/image-20250211165818757.png)
